...
std::optional<DeviceSet> DeviceAnalysis::lookupTargets(Value deviceValue) {
...
DeviceAnalysis::lookupGlobals(Value deviceValue) {
  auto globalPVS = solver.lookupElementFor<DeviceGlobalValuePVS>(
      Position::forValue(deviceValue));
  if (!globalPVS || !globalPVS->isValidState() || globalPVS->isUndefContained())
    return std::nullopt;
